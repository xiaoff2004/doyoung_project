...
{
	HAL_RTC_GetTime(nhrtc,&time,RTC_FORMAT_BIN);
	HAL_RTC_GetDate(nhrtc,&date,RTC_FORMAT_BIN);
	sAlarm.AlarmTime.Seconds = time.Seconds+1;
	if(sAlarm.AlarmTime.Seconds==60)sAlarm.AlarmTime.Seconds=0;
    HAL_RTC_SetAlarm_IT(nhrtc, &sAlarm, RTC_FORMAT_BIN);
}
/* USER CODE END 1 */
