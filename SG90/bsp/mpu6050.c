...
uint8_t MPU_WritLen(uint8_t RegAddr,uint8_t Len,uint8_t data)
{
	uint8_t result =HAL_I2C_Mem_Write(&hi2c_mpu,MPU_WRITE,RegAddr,I2C_MEMADD_SIZE_8BIT,&data,Len,1000);
	return result;
}

uint8_t MPU_ReadLen(uint8_t RegAddr,uint8_t *pdata,uint8_t Len)
{
	uint8_t result =HAL_I2C_Mem_Read(&hi2c_mpu,MPU_READ,RegAddr,I2C_MEMADD_SIZE_8BIT,pdata,Len,1000);
	return result;
}
...
short MPU_GetTemp(void)
{
	...
	short raw;
	float temp;
	MPU_ReadLen(MPU_TEMP_OUTH_REG,buff,2);
...
uint8_t MPU_GetGyro(short *gx,short *gy,short *gz)
{
	...
	result=MPU_ReadLen(MPU_GYRO_XOUTH_REG,buff,6);
	if(result==HAL_OK)
	{
...
{
	uint8_t buff[6]={0},result=0;
	result=MPU_ReadLen(MPU_ACCEL_XOUTH_REG,buff,6);
	if(result==HAL_OK)
	{
